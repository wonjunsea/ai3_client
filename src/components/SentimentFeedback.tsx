...
export const SentimentFeedback = () => {
  ...
  return <div className="bg-white rounded-xl shadow-sm p-5 border border-gray-100">
      ...
      <div className="grid grid-cols-3 gap-2 mb-6">
        {sentimentData.map((item, index) => <div key={index} className="bg-gray-50 rounded-lg p-3 text-center">
      ...
      <div className="space-y-3">
        {recentFeedback.map((feedback, index) => <div key={index} className="flex items-start p-2 border-b border-gray-100">
            <div className={`p-1.5 rounded-full mr-2 ${feedback.sentiment === 'positive' ? 'bg-green-100' : feedback.sentiment === 'negative' ? 'bg-red-100' : 'bg-yellow-100'}`}>
              <MessageSquareIcon className={`h-4 w-4 ${feedback.sentiment === 'positive' ? 'text-green-500' : feedback.sentiment === 'negative' ? 'text-red-500' : 'text-yellow-500'}`} />
            </div>
